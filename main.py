...
# Load CSV manually
file_path = "stock_data.csv"  # Update this path as needed
data = []
...
val_mse = sum((y_val[i] - val_predictions[i]) ** 2 for i in range(len(y_val))) / len(y_val)

st.write(f"Validation Mean Squared Error: {val_mse:.2f}")
...
future_predictions = [b0 + b1 * x for x in future_x]
